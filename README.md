...
M0 is sealed. M1 is actively extracting presentation-facing authority without replacing canonical game rules. The current implementation baseline is `f8eba33abc59180d5994fc70554a700cd1524a59` (`feat: qualify last-save load authority`) plus the StartMission canonical-owner extraction in this working tree. The boundary state is mechanically inventoried and qualified:

```text
strategic presentation-authoritative semantics: 57 total
  canonical-applied:                            56
  fail-closed pending owners:                   1
...
canonical verification digest:                   8add4c8c8319111766d2ba9939e6fdcab8e08bd272fbadb068d198567bcaa218
```
...
6. extract canonical strategic owners from legacy callbacks — **56 strategic semantics implemented/qualified at source-contract level so far**, covering campaign time lapse, aircraft/geoscape operations, base/installation lifecycle, facility build/destroy, Research assign/max/stop, Production create/amount/move/stop operations, the six Employee/Team assign/de-equip/delete/hire-fire/rename/skin operations, Market buy/sell aircraft/item/UGV plus explicit autosell policy, recovered/stored-UFO authority, alien-containment kill actions, aircraft equipment/rename, defence, StartTransfer, SaveGame serialization, LoadGame lifecycle normalization, and explicit-context LoadLastSave lowering;
...
10. finish the remaining mission authority family behind typed IDs and canonical owners; StartMission now uses explicit MissionId/AircraftId context with a base-attack-safe no-ID path for the existing ephemeral defender aircraft, while AutoResolveMission remains fail-closed; SaveGame, LoadGame, and LoadLastSave continue to converge on unchanged canonical v4 persistence;
...
- [x] Introduce typed presentation IDs where required.
  ...
  - [x] Qualify persisted `EmployeeId` directly from `character_t::ucn`, including duplicate-load rejection, allocator reconciliation, and preservation of the inherited nonnegative signed-16-bit wire domain.
...
- [x] Publish immutable strategic snapshots/view data without raw canonical pointers.
  ...
  - [x] Project campaign time/credits/selection plus missions, aircraft/UFOs, bases, installations, nations, technologies, employees, production queue state and messages as owning value data.
  ...
  - [x] Publish persisted `EmployeeId` with immutable base/aircraft/type/body-skin/hired/transfer/name state and no raw canonical pointers.
...
- [x] Introduce typed intent dispatch without changing canonical rules.
  ...
  - [x] Qualify **47/57 strategic semantics** through campaign-owned canonical owners: campaign time lapse; hardened aircraft mission/return/start/stop/destination/pursuit/homebase; aircraft equipment/rename; base build/rename; installation build/rename/destroy; facility build/destroy with runtime `FacilityId`; Research assign/max/stop; Production create/decrease/increase/set-amount/move/stop; Employee/Team assign/de-equip/delete/hire-fire/rename/skin; Market buy/sell aircraft/item/UGV and explicit autosell policy.
  - [x] Keep the remaining **10 strategic semantics fail-closed** until their campaign-owned validation/mutation contracts are extracted.
  ...
  - [x] Extract alien-containment authority: `KillContainedAlien` and `KillContainedAliens` now re-resolve `BaseId`/`TechnologyId` and mutate canonical aggregate containment state without inventing per-alien identity; the inherited multi-team-per-technology behavior is preserved.
  - [x] Extract aircraft-configuration authority: equipment uses the fixed `AircraftId + slotType/slotIndex/zone` structural coordinate with current `ItemId` eligibility rechecked at execution, while `RenameAircraft` preserves inherited default-name and name-validation semantics; no slot runtime ID is invented.
